"""
Build Script cho Windows Optimizer Pro v6.0 - DEBUG VERSION
Build với console window để xem output
"""
import os
import sys
import subprocess
...
def build_app():
    """Build ứng dụng thành EXE với console"""

    print("=" * 80)
    print("WINDOWS OPTIMIZER PRO v6.0 - BUILD SCRIPT (DEBUG)")
    print("=" * 80)
    print()
...
<assembly xmlns="urn:schemas-microsoft-com:asm.v1" manifestVersion="1.0">
  <assemblyIdentity
    ...
    type="win32"
  />
  <description>Windows Optimizer Pro v6.0 - Ultimate Edition (Debug)</description>
...
"""
    ...
    # Build với PyInstaller - CONSOLE VERSION
    print("[4/6] Building với PyInstaller (DEBUG - có console)...")
    ...
    build_command = [
        ...
        '--name=WindowsOptimizerPro_v6.0_DEBUG',
        '--onefile',                          # Single EXE file
        # NO --windowed và NO --noconsole để hiện console
    ...
    print()
    print("=" * 80)
    print("✓ BUILD THÀNH CÔNG! (DEBUG VERSION)")
    print("=" * 80)
    print()
    print(f"📂 File EXE: dist\\WindowsOptimizerPro_v6.0_DEBUG.exe")
    print(f"📏 Kích thước: ~{os.path.getsize('dist/WindowsOptimizerPro_v6.0_DEBUG.exe') / (1024*1024):.1f} MB")
    print()
    print("📝 Lưu ý:")
    print("   • EXE yêu cầu quyền Administrator khi chạy")
    print("   • CÓ cửa sổ console để xem output/debug")
    print("   • Đã tích hợp đầy đủ 3 folders resources")
    print("   • Dùng bản này để DEBUG và xem lỗi")
    print()
    print("🚀 Cách chạy:")
    print("   Right-click WindowsOptimizerPro_v6.0_DEBUG.exe → Run as Administrator")
    print()
...
def create_dist_readme():
    ...
    readme_content = """WINDOWS OPTIMIZER PRO v6.0 - DEBUG VERSION
==========================================

CÁCH SỬ DỤNG:
-------------
1. Right-click "WindowsOptimizerPro_v6.0_DEBUG.exe"
2. Chọn "Run as Administrator"
3. Cho phép UAC prompt (Yes)
4. Ứng dụng sẽ khởi động VỚI console window

LƯU Ý:
-------
• Ứng dụng YÊU CẦU quyền Administrator
• Có cửa sổ console để xem output và debug
• Dùng bản này để kiểm tra lỗi và debug
• Để build bản RELEASE (không có console), dùng build_app.py
...
Version 6.0.0 - Ultimate Edition (Debug)
"""

    with open('dist/README_DEBUG.txt', 'w', encoding='utf-8') as f:
        f.write(readme_content)
    print("✓ Đã tạo dist/README_DEBUG.txt")
